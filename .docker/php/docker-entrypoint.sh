...
if grep -q ^DATABASE_URL= .env; then
    ...
    if ls -A src/Infrastructure/Symfony5/Persistence/ORM/Doctrine/Migrations/*.php >/dev/null 2>&1; then
      bin/console doctrine:migrations:migrate --no-interaction
    fi
    if ls -A src/Infrastructure/Symfony5/Persistence/ORM/Fixtures/*.php >/dev/null 2>&1; then
